...
def extract_text(
    pil_img: Image.Image, use_recognize=False, allowlist=None, threshold=None
) -> str:
    ...
    if use_recognize:
        if threshold is not None:
            result = reader.recognize(
                img_np, allowlist=allowlist, text_threshold=threshold
            )
        else:
...
def sort_ocr_result(results):
    ...
    for item in sorted_results:
        ...
        if item[0][0][1] < (previous_item[0][0][1] + tolerance) and item[0][0][1] > (
            previous_item[0][0][1] - tolerance
        ):
            rows[row_number].append(item)
        else:
            row_number += 1
