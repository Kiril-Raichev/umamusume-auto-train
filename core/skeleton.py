...
def detect_scenario():
    ...
    device_action.locate_and_click(
        "assets/buttons/close_btn.png", min_search_time=get_secs(1)
    )
    sleep(0.5)
        ...
        while bot.is_bot_running:
            ...
            if constants.SCENARIO_NAME == "":
                ...
                if device_action.locate_and_click(
                    "assets/unity/unity_cup_btn.png", min_search_time=get_secs(1)
                ):
                    constants.SCENARIO_NAME = "unity"
                    info(
                        "Unity race detected, calling unity cup function. If this is not correct, please report this."
                    )
                    unity_cup_function()
                    continue
            ...
            def click_match(matches):
                if matches and len(matches) > 0:
                    ...
                    cx = x + w // 2
                    cy = y + h // 2
                    return device_action.click(
                        target=(cx, cy), text=f"Clicked match: {matches[0]}"
                    )
                return False
            ...
            if matches.get("claw_btn", False) or matches.get("claw_btn_2", False):
                ...
                if matches.get("claw_btn", False):
                    ...
                    warning(
                        "Got into claw machine match but there's no match in both versions, this should never happen."
                    )
                    continue
            ...
            if constants.SCENARIO_NAME == "unity":
                unity_matches = device_action.match_cached_templates(
                    cached_unity_templates, region_ltrb=constants.GAME_WINDOW_BBOX
                )
            ...
            if not matches.get("tazuna"):
                ...
                if constants.SCENARIO_NAME == "":
                    ...
                    info(
                        f"Scenario detected: {scenario_name}, if this is not correct, please report this."
                    )
            ...
            if state_obj["year"] == "Senior Year Late Dec":
                device_action.stop_bot(
                    "finished",
                    f"assets/notifications/{config.SUCCESS_NOTIFICATION}",
                    volume=config.NOTIFICATION_VOLUME,
            ...
            if not validate_turn(state_obj):
                info(
                    "Couldn't read turn text correctly, retrying to avoid unnecessary races. If this keeps happening please report it."
                )
                continue
            ...
            if (
                config.PRIORITIZE_MISSIONS_OVER_G1
            ...
            if (
                (not config.PRIORITIZE_MISSIONS_OVER_G1)
                and config.DO_MISSION_RACES_IF_POSSIBLE
                and state_obj["race_mission_available"]
            ):
                ...
                action["race_name"] = "any"
                action["race_image_path"] = "assets/ui/match_track.png"
                action["prioritize_missions_over_g1"] = (
                    config.PRIORITIZE_MISSIONS_OVER_G1
                )
                action["race_mission_available"] = True
                buy_skill(state_obj, action_count, race_check=True)
            ...
            training_function_name = strategy.get_training_template(state_obj)[
                "training_function"
            ]
            ...
            if isinstance(action, dict):
                error(
                    f"Strategy returned an invalid action. Please report this line. Returned structure: {action}"
                )
                ...
                if dry_run_turn:
                    ...
                    if action.available_actions:  # Check if the list is not empty
                        ...
                        warning(
                            "No more actions remaining in available_actions. Retrying turn to fix."
                        )
                        non_match_count += 1
                        continue

                    if (
                        action.options.get("race_mission_available")
                        and action.func == "do_race"
                    ):
                        info(
                            f"Couldn't match race mission to aptitudes, trying next action."
                        )
                    else:
